...
from ...interface.core import ICore
from ...detector.predictions import Predictions
from ...detector.configs import COCO_CLASSES
from ...preprocess.image.letterbox import letterbox
from ...preprocess.image.general import (basic_preprocess, bgr_to_rgb,
                                          hwc_to_chw)
from ...postprocess.bbox import scale_coords
from ...utils.backend import setup_backend
...
class Yolov5(ICore):
    ...
    def __init__(self,
        ...
        self._load_model(backend, weight, device, conf, iou, **kwargs)

    def __repr__(self):
        """Returns Backend and Model Information

        Returns:
            returns as a reference of the model since is loaded earlier upon starting the app
        """
        return self._model
    ...
    def _load_model(self, backend_name: str, weight, device: str, conf: list, iou: list, **kwargs: Any) -> None:
        ...
        if backend_name != 'tensorrt':
            self._model = Yolov5(weight, device, conf, iou)  # backend @jaime: now is just the memory reference
