...
class YoloV5sSeams:
	...
	def pre_process(self, input_image, net):
		...
		runs the forward pass and gets theoutputs layers """
...
if __name__ == '__main__':
	...
	# frame = cv2.imread('C:/Users/gomezja/PycharmProjects/201_SeamsModel/dataset/dev/ZW10A_2070_047560_TX163063.png')
	#
	# net = cv2.dnn.readNet('best_exp18_768_5c_onnx7.onnx')  # weights to cv2.dnn
	#
	# detections = yolo.pre_process(frame, net)
	# img = yolo.post_process(frame.copy(), detections, classes)
	#
	# t, _ = net.getPerfProfile()
	# label = 'Inference time: %.2f ms' % (t * 1000.0 / cv2.getTickFrequency())
	#
	# cv2.putText(img, label, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
